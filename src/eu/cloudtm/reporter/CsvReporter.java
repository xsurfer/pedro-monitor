...
public class CsvReporter {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		while (true) {
			//// -------FABIO
			// riapro il file, lo rianalizzo dopodichè controllo se ci sono ip non ancora presenti al manager e nel caso li aggiungo

			log.trace("=== RE-READING CONFIG FILE ===");
			InputStream tmpIs = openResource(configFile);
			if (tmpIs == null) {
				log.error("Cannot open the following config file '%s'", configFile);
				System.exit(1);
			}
			...
			tmpConfig.load(tmpIs);
			tmpIs.close();

			Collection<ResourceInfo> tmpResourceInfos = parseResourcesIps(tmpConfig);

			for (ResourceInfo ip : tmpResourceInfos) {
				manager.addResource(ip);
			}
